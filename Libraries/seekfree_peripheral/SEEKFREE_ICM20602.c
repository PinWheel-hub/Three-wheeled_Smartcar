...
float get_Angle_Balance1(void)
{
  ...
  float balance;
	...
	//icm_acc_y = (int16)(((uint16)buf.dat[2]<<8 | buf.dat[3]));
	icm_acc_z = (int16)(((uint16)buf.dat[4]<<8 | buf.dat[5]));
	balance=atan2(icm_acc_x,icm_acc_z);
	//oled_printf_float(0,1,balance,9,6);
	return balance;
}
//-------------------------------------------------------------------------------------------------------------------
// @brief		获取ICM20602陀螺仪数据
...
{    
    ...
    float acc_ratio = 2.8;      //加速度计比例    
    float gyro_ratio = 0.88;    //陀螺仪比例    
    float dt = 0.005;           //采样周期   
    ...
    return temp_angle;    
		...
		/*angle = K1 * angle_m+ (1-K1) * (last_angle + gyro_m * 0.005);
		last_angle=angle;*/
